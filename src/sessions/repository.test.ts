...
describe("SessionRepository provider handoff",()=>{
  test("rolling back a second handoff restores the first handoff metadata byte-for-byte across restart",()=>{const root=fresh(),cwd=join(root,"repo"),repo=new SessionRepository(root);mkdirSync(cwd);repo.reserve({taskId:"task",providerId:"claude",cwd});const first=repo.handoff({taskId:"task",providerId:"codex",reason:"first"}),before=structuredClone(first.current.handoff),ancestorBefore=structuredClone(first.previous.handoff);const second=repo.handoff({taskId:"task",providerId:"codebuddy",reason:"second"});repo.rollbackHandoff(second.current.id);const restarted=new SessionRepository(root);expect(restarted.getById(first.current.id)?.handoff).toEqual(before);expect(restarted.getById(first.previous.id)?.handoff).toEqual(ancestorBefore);expect(restarted.getByTaskId("task")?.id).toBe(first.current.id);});
  test("moves one task atomically, preserves the chain and can roll back a definite submit failure",()=>{const root=fresh(),cwd=join(root,"repo");mkdirSync(cwd);const repo=new SessionRepository(root);repo.reserve({taskId:"task",providerId:"claude",cwd,access:"workspace",extraDirs:[cwd]});const moved=repo.handoff({taskId:"task",providerId:"codex",model:"gpt-5.6-sol",reason:"quota"});expect(moved.previous).toMatchObject({providerId:"claude",taskIds:[],handoff:{successorId:moved.current.id,status:"superseded"}});expect(moved.current).toMatchObject({providerId:"codex",nativeRef:null,taskIds:["task"],cwd,access:"workspace",extraDirs:[cwd],handoff:{predecessorId:moved.previous.id,status:"active",reason:"quota"}});expect(new SessionRepository(root).getByTaskId("task")?.id).toBe(moved.current.id);expect(repo.rollbackHandoff(moved.current.id)).toMatchObject({providerId:"claude",taskIds:["task"]});expect(new SessionRepository(root).getById(moved.previous.id)?.handoff).toBeUndefined();expect(new SessionRepository(root).getById(moved.current.id)).toMatchObject({taskIds:[],handoff:{status:"failed"}});});
  test("same-provider handoff leaves the repository byte-identical",()=>{const root=fresh(),cwd=join(root,"repo");mkdirSync(cwd);const repo=new SessionRepository(root);repo.reserve({taskId:"task",providerId:"claude",cwd});const file=join(root,"kernel/sessions.json"),before=readFileSync(file,"utf8");expect(()=>repo.handoff({taskId:"task",providerId:"claude"})).toThrow("SESSION_HANDOFF_SAME_PROVIDER");expect(readFileSync(file,"utf8")).toBe(before);});
});
