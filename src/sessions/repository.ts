...
export class SessionRepository {
  ...
  handoff(input:{taskId:string;providerId:SessionProviderId;model?:string;effort?:string;reason?:string}):{previous:SessionRecord;current:SessionRecord}{if(!input.taskId||input.reason!==undefined&&!validOptionText(input.reason,512))throw new SessionRepositoryError("handoff 参数非法");const file=fileFor(this.dataRoot),expected=existsSync(file)?readFileSync(file,"utf8"):null,view=mutableView(this.dataRoot),store=view.store;assertNotIsolated(view,input.taskId);const previous=store.sessions.find(s=>s.taskIds.includes(input.taskId));if(!previous)throw new SessionRepositoryError(`Session 不存在: ${input.taskId}`);assertMutable(previous);if(previous.providerId===input.providerId)throw new SessionRepositoryError("SESSION_HANDOFF_SAME_PROVIDER");const at=new Date().toISOString(),id=`${input.taskId}:handoff:${crypto.randomUUID()}`;previous.taskIds=previous.taskIds.filter(x=>x!==input.taskId);previous.updatedAt=at;previous.handoff={...(previous.handoff?.predecessorId?{predecessorId:previous.handoff.predecessorId}:{}),successorId:id,at,...(input.reason?{reason:input.reason}:{}),status:"superseded"};store.sessions.push({id,providerId:input.providerId,nativeRef:null,previousRefs:[],cwd:previous.cwd,control:previous.control,taskIds:[input.taskId],recoverable:false,source:"native",createdAt:at,updatedAt:at,...(previous.access?{access:previous.access}:{}),...(previous.extraDirs?.length?{extraDirs:[...previous.extraDirs]}:{}),...(input.model?{model:input.model}:{}),...(input.effort?{effort:input.effort}:{}),handoff:{predecessorId:previous.id,at,...(input.reason?{reason:input.reason}:{}),status:"active"}});atomicWrite(this.dataRoot,mutationPayload(view,store),expected);const repo=new SessionRepository(this.dataRoot);return{previous:repo.getById(previous.id)!,current:repo.getByTaskId(input.taskId)!};}
