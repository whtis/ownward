...
export interface SkillRouteContext { browserSession?: { id: string; interactive: boolean; denial?: string | null }; confirmUserPresence?: (sessionId: string) => Promise<boolean>; approvals?: ApprovalStore }
async function trustedSession(context: SkillRouteContext | undefined, requirePresence: boolean): Promise<string> { const session = context?.browserSession; if (!session?.id || (requirePresence && session.interactive !== true)) throw Object.assign(new Error(`只有经过同源验证的交互式设置页可以执行此操作：${session?.denial || "请求没有带上浏览器会话"}`), { code: "SKILL_APPROVAL_INTERACTIVE_REQUIRED" }); if (requirePresence && (!context?.confirmUserPresence || await context.confirmUserPresence(session.id) !== true)) throw Object.assign(new Error("未确认本机用户在场"), { code: "SKILL_APPROVAL_PRESENCE_REQUIRED" }); return session.id; }
