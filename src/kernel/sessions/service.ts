...
export class KernelSessionService implements SessionService {
  ...
  private session(id: string): SessionRecord {
    const s = this.repo.getById(id) ?? this.repo.getByTaskId(id); if (!s) throw new KernelSessionPolicyError("SESSION_NOT_FOUND", `Session 不存在: ${id}`);
