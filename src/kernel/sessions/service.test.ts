...
import { archiveRunnerEvents, RunnerCommandJournal, RunnerEventJournal } from "../../runner/journals.ts";
import { KernelSessionService, readStableRunnerSnapshot, shellQuote, SESSION_STATE_CACHE_LIMIT } from "./service.ts";
...
import { pruneBridgeCommands, SessionRunnerBridgeStore } from "./bridge-store.ts";
...
describe("Kernel SessionService migration modes", () => {
  ...
  test("per-service state cache is bounded and disposable",async()=>{const root=fresh(),repo=new SessionRepository(root),service=new KernelSessionService(root);for(let i=0;i<SESSION_STATE_CACHE_LIMIT+8;i++){const cwd=join(root,`repo-${i}`);mkdirSync(cwd);repo.reserve({taskId:`state-${i}`,providerId:"claude",cwd});await service.state(`state-${i}`);}expect(service.cacheSizeForTest()).toBe(SESSION_STATE_CACHE_LIMIT);service.dispose();expect(service.cacheSizeForTest()).toBe(0);});
  test("per-session signature: appends to another session keep the cache; same-session appends only apply the new events", async () => {
    const root=fresh(),cwd=join(root,"repo");mkdirSync(cwd);const repo=new SessionRepository(root);repo.reserve({taskId:"inc-a",providerId:"claude",cwd});repo.reserve({taskId:"inc-b",providerId:"claude",cwd});
    const commands=new RunnerCommandJournal(root),events=new RunnerEventJournal(root),service=new KernelSessionService(root);
    const turn=(session:string,n:number)=>{const c=commands.accept({commandId:`${session}-c${n}`,kind:"start-run",runId:`${session}-r${n}`,sessionId:session,providerId:"claude",input:JSON.stringify({text:`q${n}`})},`2026-09-13T00:00:0${n}.000Z`).record;events.append({eventId:`${session}-s${n}`,type:"started",at:`2026-09-13T00:00:0${n}.100Z`,commandId:c.commandId,runId:c.runId,sessionId:session,providerId:"claude"});events.append({eventId:`${session}-m${n}`,type:"message-completed",at:`2026-09-13T00:00:0${n}.500Z`,commandId:c.commandId,runId:c.runId,sessionId:session,providerId:"claude",payload:JSON.stringify({role:"assistant",text:`a${n}`})});};
    turn("inc-a",1);turn("inc-b",1);
    const a1=await service.state("inc-a");expect(a1.messages.map((m:any)=>m.text)).toEqual(["q1","a1"]);expect(service.projectionStatsForTest().rebuilds).toBe(1);
    // 另一个会话追加：inc-a 的签名不变，缓存直接命中，不重建也不增量
    turn("inc-b",2);await service.state("inc-a");expect(service.projectionStatsForTest().rebuilds).toBe(1);
    // 自己追加一轮：同一个投影器只 apply 新事件（rebuilds 不涨），消息完整
    turn("inc-a",2);const a2=await service.state("inc-a");expect(a2.messages.map((m:any)=>m.text)).toEqual(["q1","a1","q2","a2"]);expect(service.projectionStatsForTest().rebuilds).toBe(1);
    // 初始历史落盘（历史指纹变）→ 必须整个重建，且历史排在前面
    writeInitialHistory(root,{sessionId:"inc-a",providerId:"claude",nativeRef:"native-inc-a",messages:[{role:"user",text:"h0",ts:"2026-09-12T00:00:00.000Z"}]});
    const a3=await service.state("inc-a");expect(a3.messages.map((m:any)=>m.text)).toEqual(["h0","q1","a1","q2","a2"]);expect(service.projectionStatsForTest().rebuilds).toBe(2);
    // 索引投影：签名相同即可跳过；不截断、不问 runner
    const view=service.projectForIndex("inc-a")!;expect(view.state.messages.length).toBe(5);expect(service.projectForIndex("inc-a")!.signature).toBe(view.signature);expect(service.indexSignature("inc-a")).toBe(view.signature);
    turn("inc-a",3);expect(service.indexSignature("inc-a")).not.toBe(view.signature);
  });
  test("/new 之后的历史重置同样截断归档：重置点在热 journal、在归档、或整段被归档，都只看到重置之后的对话", async () => {
    const root=fresh(),cwd=join(root,"repo");mkdirSync(cwd);const repo=new SessionRepository(root);repo.reserve({taskId:"reset-arc",providerId:"claude",cwd});
    const commands=new RunnerCommandJournal(root),events=new RunnerEventJournal(root),service=new KernelSessionService(root);
    const turn=(n:number,day:string)=>{const c=commands.accept({commandId:`c${n}`,kind:"start-run",runId:`r${n}`,sessionId:"reset-arc",providerId:"claude",input:JSON.stringify({text:`q${n}`})},`${day}T00:00:00.000Z`).record;events.append({eventId:`s${n}`,type:"started",at:`${day}T00:00:00.100Z`,commandId:c.commandId,runId:c.runId,sessionId:"reset-arc",providerId:"claude"});events.append({eventId:`m${n}`,type:"message-completed",at:`${day}T00:00:01.000Z`,commandId:c.commandId,runId:c.runId,sessionId:"reset-arc",providerId:"claude",payload:JSON.stringify({role:"assistant",text:`a${n}`})});events.append({eventId:`d${n}`,type:"completed",at:`${day}T00:00:02.000Z`,commandId:c.commandId,runId:c.runId,sessionId:"reset-arc",providerId:"claude"});};
    turn(1,"2026-01-01");
    const reset=commands.accept({commandId:"reset",kind:"new-session",runId:"r-reset",sessionId:"reset-arc",providerId:"claude",input:"{}"},"2026-01-02T00:00:00.000Z").record;repo.beginHistoryReset("reset-arc","reset");
    events.append({eventId:"reset-s",type:"started",at:"2026-01-02T00:00:00.100Z",commandId:reset.commandId,runId:reset.runId,sessionId:"reset-arc",providerId:"claude"});events.append({eventId:"reset-d",type:"completed",at:"2026-01-02T00:00:01.000Z",commandId:reset.commandId,runId:reset.runId,sessionId:"reset-arc",providerId:"claude"});repo.finishHistoryReset("reset-arc","reset",true);
    turn(2,"2026-01-03");
    const texts=async()=>(await service.state("reset-arc")).messages.map((m:any)=>m.text);
    expect(await texts()).toEqual(["q2","a2"]);                                                   // 重置点在热 journal
    expect(archiveRunnerEvents(root,{retentionDays:1,now:new Date("2026-01-02T12:00:00.000Z")}).sessions).toEqual(["reset-arc"]);   // 只归档 turn1
    expect(await texts()).toEqual(["q2","a2"]);                                                   // 归档里的重置前对话不能复活
    expect(archiveRunnerEvents(root,{retentionDays:1,now:new Date("2026-01-10T00:00:00.000Z")}).archived).toBeGreaterThan(0);          // 重置点和 turn2 也进归档
    expect(await texts()).toEqual(["q2","a2"]);                                                   // 重置点在归档：归档按它截断，热 journal 为空
    expect(service.projectionStatsForTest().rebuilds).toBe(3);                                    // 归档文件每变一次重建一次
  });
  ...
  test("runner state uses CodeBuddy rather than Codex for CodeBuddy resume",async()=>{const root=fresh(),cwd=join(root,"repo"),nativeRef="codebuddy-session";mkdirSync(cwd);new SessionRepository(root).bind({taskId:"codebuddy",providerId:"codebuddy",nativeRef,cwd});const resume=(await new KernelSessionService(root).state("codebuddy")).resume;expect(resume).toEqual({id:nativeRef,tool:"codebuddy",cmd:`cd '${cwd}' && codebuddy --resume '${nativeRef}'`});});
  ...
  test("Kernel validates model/effort and expands only granted Codex homes", async()=>{
    ...
    await expect(service.create({taskId:"bad-claude-effort",providerId:"claude",cwd,effort:"minimal"},{roots:[root],access:"workspace"})).rejects.toMatchObject({code:"PROVIDER_INPUT_INVALID"});
    await expect(service.create({taskId:"bad-codex-effort",providerId:"codex",cwd,effort:"minimal"},{roots:[root],access:"workspace"})).rejects.toMatchObject({code:"PROVIDER_INPUT_INVALID"});
    await expect(service.create({taskId:"bad-luna-ultra",providerId:"codex",cwd,model:"gpt-5.6-luna",effort:"ultra"},{roots:[root],access:"workspace"})).rejects.toMatchObject({code:"PROVIDER_INPUT_INVALID"});
    await expect(service.create({taskId:"unknown-with-effort",providerId:"codex",cwd,model:"gpt-explicit",effort:"high"},{roots:[root],access:"workspace"})).rejects.toMatchObject({code:"PROVIDER_INPUT_INVALID"});
    const defaultCodex=await service.create({taskId:"default-codex",providerId:"codex",cwd,effort:"max"},{roots:[root],access:"workspace"});expect(defaultCodex).toMatchObject({model:"gpt-5.6-sol",effort:"max"});expect(await service.state(defaultCodex.id)).toMatchObject({model:"gpt-5.6-sol",effort:"max"});
    const ultraCodex=await service.create({taskId:"ultra-codex",providerId:"codex",cwd,model:"gpt-5.6-terra",effort:"ultra"},{roots:[root],access:"workspace"});expect(ultraCodex).toMatchObject({model:"gpt-5.6-terra",effort:"ultra"});
    const explicitCodex=await service.create({taskId:"explicit-codex",providerId:"codex",cwd,model:"gpt-explicit"},{roots:[root],access:"workspace"});expect(explicitCodex).toMatchObject({model:"gpt-explicit"});expect(explicitCodex.effort).toBeUndefined();
    const deepClaude=await service.create({taskId:"deep-claude",providerId:"claude",cwd,effort:"max"},{roots:[root],access:"workspace"});expect(deepClaude).toMatchObject({effort:"max"});expect(deepClaude.model).toBeUndefined();
  ...
  test("same-provider model/effort change reconfigures in place (no successor, native ref kept) while identical options stay rejected",async()=>{const root=fresh(),cwd=join(root,"repo"),repo=new SessionRepository(root);mkdirSync(cwd);const original=repo.reserve({taskId:"task",providerId:"codex",cwd,model:"gpt-5.6-sol",effort:"low"});repo.bind({taskId:"task",providerId:"codex",nativeRef:"019ffae9-ad07-7ef0-ab0a-761b9a426650",cwd,control:"ownward"});const seen:string[]=[],provider:RunnerProvider={async *execute(command,input){seen.push(`${command.kind}:${input}`);yield{eventId:`${command.commandId}:started`,type:"started",at:new Date().toISOString(),commandId:command.commandId,runId:command.runId,sessionId:command.sessionId,providerId:command.providerId};yield{eventId:`${command.commandId}:done`,type:"completed",at:new Date().toISOString(),commandId:command.commandId,runId:command.runId,sessionId:command.sessionId,providerId:command.providerId};}},server=new RunnerServer(root,()=>provider);server.start();try{const service=new KernelSessionService(root,{mode:"runner",roots:[root]});await expect(service.handoff("task",{providerId:"codex",model:"gpt-5.6-sol",effort:"low"})).rejects.toMatchObject({code:"SESSION_HANDOFF_SAME_PROVIDER"});await expect(service.reconfigure("task",{effort:"low"})).rejects.toMatchObject({code:"SESSION_RECONFIGURE_NOOP"});await expect(service.reconfigure("task",{model:"gpt-5.5",effort:"ultra"})).rejects.toMatchObject({code:"PROVIDER_INPUT_INVALID"});
    const result=await service.handoff("task",{providerId:"codex",effort:"xhigh"}),current=new SessionRepository(root).getByTaskId("task")!;expect(result).toMatchObject({queued:false,sessionId:original.id,providerId:"codex",inPlace:true});expect(current).toMatchObject({id:original.id,model:"gpt-5.6-sol",effort:"xhigh",nativeRef:"019ffae9-ad07-7ef0-ab0a-761b9a426650"});expect(current.handoff).toBeUndefined();expect(new SessionRepository(root).list()).toHaveLength(1);
    // 只下发真正变化的键；Provider 收到的是 set-options 而不是新会话首轮
    expect(seen).toEqual(['set-options:{"effort":"xhigh"}']);
    const direct=await service.reconfigure("task",{model:"gpt-6-astra"});expect(direct).toMatchObject({sessionId:original.id,providerId:"codex",model:"gpt-6-astra",effort:"xhigh"});expect(new SessionRepository(root).getByTaskId("task")).toMatchObject({model:"gpt-6-astra",effort:"xhigh"});expect(seen.at(-1)).toBe('set-options:{"model":"gpt-6-astra"}');
    const state=await service.state("task");expect(state).toMatchObject({model:"gpt-6-astra",effort:"xhigh"});expect(state.handoff).toBeUndefined();expect(state.messages.some(message=>message.name==="handoff")).toBeFalse();}finally{server.stop();}});
  test("reconfigure definite Runner failure leaves persisted options unchanged; observing control and pending approvals are refused",async()=>{const root=fresh(),cwd=join(root,"repo"),repo=new SessionRepository(root);mkdirSync(cwd);repo.reserve({taskId:"cfg",providerId:"claude",cwd,model:"sonnet",effort:"medium"});const success:any={require(){},submit:async(_task:any,_session:any,_kind:any,_input:any,identity:any)=>identity,waitTerminal:async()=>[],syncCommand:async()=>[]};const failed:any={...success,waitTerminal:async()=>{throw Object.assign(new Error("definite"),{code:"RUNNER_CONTROL_FAILED",outcomeUnknown:false});}};await expect(new KernelSessionService(root,{mode:"runner",roots:[root]},failed).reconfigure("cfg",{model:"opus"})).rejects.toThrow("definite");expect(new SessionRepository(root).getByTaskId("cfg")).toMatchObject({model:"sonnet",effort:"medium"});
    const service=new KernelSessionService(root,{mode:"runner",roots:[root]},success);await service.reconfigure("cfg",{model:"opus",effort:"high"});expect(new SessionRepository(root).getByTaskId("cfg")).toMatchObject({model:"opus",effort:"high"});
    repo.setControl("cfg","observing");await expect(service.reconfigure("cfg",{model:"haiku"})).rejects.toMatchObject({code:"SESSION_CONTROL_REQUIRED"});expect(new SessionRepository(root).getByTaskId("cfg")?.model).toBe("opus");});

  test("concurrent handoffs CAS the same predecessor and submit exactly one successor",async()=>{
    const root=fresh(),cwd=join(root,"repo"),repo=new SessionRepository(root);mkdirSync(cwd);const original=repo.reserve({taskId:"task",providerId:"claude",cwd});let active=0,maxActive=0;const executed:string[]=[];
    const provider:RunnerProvider={async *execute(command){active++;maxActive=Math.max(maxActive,active);executed.push(command.sessionId);try{yield{eventId:`${command.commandId}:started`,type:"started",at:new Date().toISOString(),commandId:command.commandId,runId:command.runId,sessionId:command.sessionId,providerId:command.providerId};await Bun.sleep(30);yield{eventId:`${command.commandId}:done`,type:"completed",at:new Date().toISOString(),commandId:command.commandId,runId:command.runId,sessionId:command.sessionId,providerId:command.providerId};}finally{active--;}}},server=new RunnerServer(root,()=>provider);server.start();
    try{const service=new KernelSessionService(root,{mode:"runner",roots:[root]}),results=await Promise.allSettled([service.handoff("task",{providerId:"codex",reason:"first"}),service.handoff("task",{providerId:"codebuddy",reason:"second"})]);await Bun.sleep(80);expect(results.filter(result=>result.status==="fulfilled")).toHaveLength(1);const rejected=results.find(result=>result.status==="rejected") as PromiseRejectedResult;expect(rejected.reason).toMatchObject({code:"SESSION_HANDOFF_STALE"});const sessions=new SessionRepository(root).list(),current=sessions.find(session=>session.taskIds.includes("task"))!;expect(current.handoff?.predecessorId).toBe(original.id);expect(sessions.filter(session=>session.handoff?.predecessorId===original.id)).toHaveLength(1);expect(sessions.filter(session=>session.taskIds.includes("task"))).toHaveLength(1);expect(new RunnerCommandJournal(root).readStrict()).toHaveLength(1);expect(executed).toEqual([current.id]);expect(maxActive).toBe(1);}finally{server.stop();}
  });

  test("definite handoff submit rejection abandons bridge before rollback and permits later writes",async()=>{
    const root=fresh(),cwd=join(root,"repo"),repo=new SessionRepository(root);mkdirSync(cwd);const original=repo.reserve({taskId:"task",providerId:"claude",cwd}),client:any={request:async()=>{throw Object.assign(new Error("rejected"),{code:"RUNNER_COMMAND_CONFLICT"});},close(){}};
    const service=new KernelSessionService(root,{mode:"runner",roots:[root]},new RunnerSessionConsumer(root,()=>client,()=>"handoff-rejected"));
    await expect(service.handoff("task",{providerId:"codex"})).rejects.toMatchObject({code:"RUNNER_COMMAND_CONFLICT",outcomeUnknown:false});
    const command=new SessionRunnerBridgeStore(root).list().find(item=>item.errorCode==="RUNNER_COMMAND_CONFLICT")!;expect(command).toMatchObject({terminal:true,errorCode:"RUNNER_COMMAND_CONFLICT"});expect(new SessionRepository(root).getByTaskId("task")?.id).toBe(original.id);expect((new KernelSessionService(root,{mode:"off"}) as any).pendingLegacyRunner(undefined,"task")).toBeFalse();expect(repo.setControl("task","observing").control).toBe("observing");expect(repo.setControl("task","ownward").control).toBe("ownward");
  });

  test("handoff projection traverses more than 15 predecessors and marks message-budget truncation",async()=>{
    const root=fresh(),cwd=join(root,"repo"),repo=new SessionRepository(root);mkdirSync(cwd);let current=repo.reserve({taskId:"task",providerId:"claude",cwd});writeInitialHistory(root,{sessionId:current.id,providerId:current.providerId,nativeRef:"history-0",messages:[{role:"user",text:"origin-message",ts:"2026-08-01T00:00:00.000Z"}]});
    const providers=["codex","codebuddy","claude"] as const;for(let i=1;i<=18;i++){const next=providers[(i-1)%providers.length]!;current=repo.handoff({taskId:"task",expectedSessionId:current.id,providerId:next,reason:`hop-${i}`}).current;writeInitialHistory(root,{sessionId:current.id,providerId:current.providerId,nativeRef:`history-${i}`,messages:[{role:"assistant",text:`message-${i}`,ts:`2026-08-01T00:${String(i).padStart(2,"0")}:00.000Z`}]});}
    const service=new KernelSessionService(root,{mode:"runner"}),states=[await service.state("task"),(await service.states(["task"])).get("task")!,await new KernelSessionService(root,{mode:"runner"}).state("task")];for(const state of states){expect(state.messages.some(message=>message.text==="origin-message")).toBeTrue();expect(state.messages.filter(message=>message.name==="handoff")).toHaveLength(18);expect(state.messages.some(message=>message.name==="history")).toBeFalse();}
    const bounded=repo.reserve({taskId:"bounded",providerId:"claude",cwd});writeInitialHistory(root,{sessionId:bounded.id,providerId:"claude",nativeRef:"bounded-history",messages:Array.from({length:250},(_,index)=>({role:"assistant" as const,text:`bounded-${index}-${"x".repeat(600)}`,ts:new Date(1700000000000+index).toISOString()}))});const state=await new KernelSessionService(root,{mode:"runner"}).state("bounded");expect(state.messages[0]).toMatchObject({role:"system",name:"history"});expect(state.messages[0].text).toContain("较早消息已省略");expect(state.messages.at(-1)?.text).toContain("bounded-249-");expect(state.messages.length).toBeLessThanOrEqual(201);
  });
...
describe("bridge 台账剪枝", () => {
  const DAY = 86_400_000, now = new Date("2026-09-09T00:00:00.000Z");
  const ago = (days: number) => new Date(now.getTime() - days * DAY).toISOString();
  const cmd = (over: Partial<any> & { commandId: string }) => ({
    runId: `run-${over.commandId}`, taskId: "t1", sessionId: "s1", providerId: "claude", kind: "send-input",
    inputHash: "h".repeat(64), cursor: 0, terminal: true, createdAt: ago(30), ...over,
  });
  const seed = (commands: any[]) => { const root = fresh(); writeFileSync(join(root, "session-runner-bridge.json"), JSON.stringify({ schemaVersion: 1, commands }, null, 2) + "\n"); return root; };
  const idsIn = (root: string) => new SessionRunnerBridgeStore(root).list().map((c) => c.commandId).sort();

  test("只剪窗口外的已终结命令，未终结的一律不动", () => {
    const root = seed([
      cmd({ commandId: "old-done", createdAt: ago(30) }),                    // 旧且已终结 → 该剪
      cmd({ commandId: "old-live", createdAt: ago(30), terminal: false }),   // 旧但还在跑 → 留
      cmd({ commandId: "recent", createdAt: ago(1) }),                       // 窗口内 → 留（也是 s1 最新）
      cmd({ commandId: "other-session", sessionId: "s2", createdAt: ago(40) }), // s2 唯一一条 → 留
    ]);
    expect(pruneBridgeCommands(root, { retentionDays: 7, now })).toMatchObject({ pruned: 1, kept: 3 });
    expect(idsIn(root)).toEqual(["old-live", "other-session", "recent"].sort());
  });

  test("接力命令永不剪：会话历史靠它们把内部消息藏起来", () => {
    const root = seed([
      cmd({ commandId: "handoff-old", createdAt: ago(90), clientMutationId: "handoff:abc" }),
      cmd({ commandId: "plain-old", createdAt: ago(90) }),
      cmd({ commandId: "newest", createdAt: ago(1) }),
    ]);
    pruneBridgeCommands(root, { retentionDays: 7, now });
    expect(idsIn(root)).toEqual(["handoff-old", "newest"]);                  // plain-old 走了，接力的留下
  });

  test("每个会话最少留一条：send() 靠 list(sessionId).length 判断会话有没有过命令", () => {
    const root = seed([
      cmd({ commandId: "s1-older", sessionId: "s1", createdAt: ago(90) }),
      cmd({ commandId: "s1-newer", sessionId: "s1", createdAt: ago(60) }),
      cmd({ commandId: "s2-only", sessionId: "s2", createdAt: ago(90) }),
    ]);
    pruneBridgeCommands(root, { retentionDays: 7, now });
    expect(idsIn(root)).toEqual(["s1-newer", "s2-only"]);                    // 两个会话各剩最新的一条
    for (const id of ["s1", "s2"]) expect(new SessionRunnerBridgeStore(root).list(id).length).toBeGreaterThan(0);
  });

  test("没得剪时一个字节都不写（避免每天白白重写整份文件）", () => {
    const root = seed([cmd({ commandId: "recent", createdAt: ago(1) })]);
    const file = join(root, "session-runner-bridge.json"), before = readFileSync(file, "utf8");
    expect(pruneBridgeCommands(root, { retentionDays: 7, now })).toMatchObject({ pruned: 0 });
    expect(readFileSync(file, "utf8")).toBe(before);
  });

  test("剪完的文件仍能通过严格校验，活跃命令的 cursor/terminal 原样保留", () => {
    const root = seed([
      cmd({ commandId: "old-done", createdAt: ago(30) }),
      cmd({ commandId: "live", createdAt: ago(20), terminal: false, cursor: 42 }),   // 更新，故「每会话最新」钉的是它
    ]);
    pruneBridgeCommands(root, { retentionDays: 7, now });
    const left = new SessionRunnerBridgeStore(root).list();
    expect(left).toHaveLength(1);
    expect(left[0]).toMatchObject({ commandId: "live", cursor: 42, terminal: false });
  });
});
