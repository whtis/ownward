...
const opts = { access: "workspace-write" as const, extraDirs: [] as string[], home: { kind: "default" as const }, model: "gpt-5.5", effort: "high" as const };
...
describe("Codex Runner Provider contract", () => {
  ...
  test("set-options 就地换模型/思考深度：下一轮 resume 同一 thread 且带新参数；非法组合在 started 前拒绝；无内存态直接 completed", async () => { const { data, server, client } = await fixture(); try {
    await client.request("submit", body("base", "one", "shared")); await terminal(client, "base");
    await client.request("submit", { commandId: "opts", kind: "set-options", runId: "r-opts", sessionId: "shared", providerId: "codex", input: JSON.stringify({ model: "gpt-5.6-luna", effort: "max" }) }); expect((await terminal(client, "opts")).at(-1)).toMatchObject({ type: "completed" });
    await client.request("submit", { commandId: "after", kind: "send-input", runId: "r-after", sessionId: "shared", providerId: "codex", input: JSON.stringify({ text: "two", images: [] }) }); const p = JSON.stringify(payloads(data, await terminal(client, "after")));
    expect(p).toContain(`resume|--|${NATIVE}|two`); expect(p).toContain("--model|gpt-5.6-luna"); expect(p).toContain('model_reasoning_effort=\\"max\\"');
    // Kernel 带新 options 的恢复快照续聊，身份比对必须通过
    await client.request("submit", { commandId: "recover", kind: "send-input", runId: "r-recover", sessionId: "shared", providerId: "codex", input: JSON.stringify({ text: "three", images: [], cwd: tmpdir(), options: { ...opts, model: "gpt-5.6-luna", effort: "max" }, nativeRef: NATIVE }) }); expect((await terminal(client, "recover")).at(-1)).toMatchObject({ type: "completed" });
    await client.request("submit", { commandId: "bad", kind: "set-options", runId: "r-bad", sessionId: "shared", providerId: "codex", input: JSON.stringify({ effort: "ultra" }) }); const bad = await terminal(client, "bad"); expect(bad.at(-1)).toMatchObject({ type: "failed", reason: "provider_input_invalid" }); expect(bad.some((e) => e.type === "started")).toBe(false);
    await client.request("submit", { commandId: "ghost", kind: "set-options", runId: "r-ghost", sessionId: "never", providerId: "codex", input: JSON.stringify({ model: "gpt-5.5" }) }); expect((await terminal(client, "ghost")).at(-1)).toMatchObject({ type: "completed" });
    // Runner 重启后没有内存态：解除沙箱 / 加目录同样直接 completed（下一轮 resume 携带 Kernel 持久化的 options），非法输入仍拒绝
    await client.request("submit", { commandId: "ghost-access", kind: "set-access", runId: "r-ghost-access", sessionId: "never", providerId: "codex", input: JSON.stringify({ access: "full-access" }) }); expect((await terminal(client, "ghost-access")).at(-1)).toMatchObject({ type: "completed" });
    await client.request("submit", { commandId: "ghost-dir", kind: "add-dir", runId: "r-ghost-dir", sessionId: "never", providerId: "codex", input: JSON.stringify({ dir: tmpdir() }) }); expect((await terminal(client, "ghost-dir")).at(-1)).toMatchObject({ type: "completed" });
    await client.request("submit", { commandId: "ghost-bad", kind: "set-access", runId: "r-ghost-bad", sessionId: "never", providerId: "codex", input: JSON.stringify({ access: "root" }) }); const ghostBad = await terminal(client, "ghost-bad"); expect(ghostBad.at(-1)).toMatchObject({ type: "failed" }); expect(ghostBad.some((e) => e.type === "started")).toBe(false);
  ...
  test("终态之后、清理完成之前的窗口不再判忙：排队消息不会被吞", async () => {
    // 线上实撞（2026-08-31）：上一轮终态 11:20:10.317 落盘，队列 flush .441 就把排队那条发下来，
    // .464 被回 PROVIDER_SESSION_BUSY —— run 立刻 failed 且一个字没产出，而命令已经进 journal，
    // 消息照样上屏：用户看着自己发出去了，agent 从没收到。claude adapter 早有这道等待，codex 补齐。
    const { data, server, client } = await fixture(); try {
      await client.request("submit", body("linger", "LINGER_AFTER_DONE", "window-session"));
      expect((await terminal(client, "linger")).at(-1)).toMatchObject({ type: "completed" });
      // 终态刚落盘就发下一轮：fake CLI 还要再活 400ms，正好是 finally 收尾的那段窗口
      await client.request("submit", { ...body("after", "second", "window-session"), kind: "resume-run", input: JSON.stringify({ text: "second", cwd: tmpdir(), images: [], options: opts, nativeRef: NATIVE }) });
      const events = await terminal(client, "after", 6_000);
      expect(events.at(-1)).toMatchObject({ type: "completed" });
      expect(events.some((event: any) => event.type === "failed")).toBeFalse();
      expect(JSON.stringify(payloads(data, events))).toContain("reply:second");
    } finally { client.close(); server.stop(); }
  });
